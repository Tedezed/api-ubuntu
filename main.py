...
def main():
    global pokemonName, pokemonLocation, pokemonTime
    ...
    while True:
        ...
        for poke in visible:
            ...
            pokemonName.append(pokemons[poke.pokemon.PokemonId - 1]['Name'])
            pokemonLocation.append(str(poke.Latitude) + ', ' + str(poke.Longitude))
            pokemonTime.append(str(poke.TimeTillHiddenMs / 1000))
            counter+=1
            if(counter == len(visible)):
                for name in pokemonName:
                    for location in pokemonLocation:
                        for time in pokemonTime:
                            dict1 = {"name" : name, "location" : location, "time" : time}
                            dictList.append(dict1)
                return Response(json.dumps(dictList), mimetype = 'application/json')
